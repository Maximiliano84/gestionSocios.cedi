...
export default function CarnetDigital({
  ...
  const ok = estado === "activo" && estadoCuota === "al_dia";
  const estadoTexto = ok
    ? "AL DÍA"
    : estado === "baja"
      ? "BAJA"
      : estadoCuota === "con_deuda"
        ? "CON DEUDA"
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 h-full px-5 pt-5 pb-8 flex flex-col gap-3">
        <div className="grid grid-cols-[110px_1fr_110px] items-start gap-4">
          ...
          <div className="text-center px-1 min-w-0 pt-1">
            <div className="text-[11px] uppercase tracking-[0.28em] text-white/70 font-semibold leading-none">
              {nombreClub}
            </div>
            <div className="mt-2 text-[21px] font-black leading-tight text-white" style={{ fontFamily: "Outfit, sans-serif" }}>
              {titulo}
            </div>
            <div className="mt-2 flex justify-center">
              <span className={`inline-flex items-center rounded-full border px-3 py-1 text-[11px] font-black tracking-wide ${ok ? theme.chipOk : theme.chipWarn}`}>
                {estadoTexto}
              </span>
            </div>
          </div>
