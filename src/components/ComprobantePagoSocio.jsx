...
function buildWhatsAppMessage({ socio }) {
  const nombreCompleto = [socio?.nombre, socio?.apellido].filter(Boolean).join(" ") || "el socio";

  return [
    `Hola, familia. Les compartimos el comprobante de pago de la cuota social de ${nombreCompleto}.`,
    "",
...
export default function ComprobantePagoSocioDialog({ open, onOpenChange, socio, pago, cfg, onFinish }) {
  const comprobanteRef = useRef(null);
  const [confirmDownload, setConfirmDownload] = useState(false);
  const [feedback, setFeedback] = useState(null);

  if (!socio || !pago) return null;

  const nombreCompleto = `${socio.nombre || ""} ${socio.apellido || ""}`.trim();
  const mesesLabel = getMesesLabel(pago);
  const logoSrc = cfg?.logoUrl || "/logo-cedi.png";
  ...
  const downloadPNG = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${cleanFileName(`comprobante-${socio.numeroSocio || "socio"}-${socio.apellido || "cedi"}-${pago.fechaPago || ""}`)}.png`;
      a.click();
  ...
  const sendWhatsApp = () => {
    const message = buildWhatsAppMessage({ socio });
    const phone = getPhone(socio.tutorTelefono);
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm text-slate-600">
                    Se generó el comprobante de pago para {nombreCompleto || "el socio"}.
                  </p>
                </div>
              </div>
            </DialogHeader>
              ...
              <div ref={comprobanteRef} className="w-full max-w-[520px] bg-white p-6 text-slate-900">
                <div className="flex items-start justify-between gap-4 border-b border-slate-200 pb-5">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-xs font-bold uppercase tracking-[0.25em] text-blue-700">{cfg?.nombreClub || "CEDI LOS 15"}</div>
                      <h2 className="mt-1 text-2xl font-black text-slate-950" style={{ fontFamily: "Outfit, sans-serif" }}>
                        Comprobante de pago
                      </h2>
                      <p className="mt-1 text-xs font-semibold uppercase tracking-wider text-slate-500">Cuota social · Fútbol</p>
                    </div>
                  </div>
                  <div className="rounded-xl bg-blue-50 px-3 py-2 text-right">
                    <div className="text-[10px] font-bold uppercase tracking-wider text-blue-600">Fecha</div>
                    <div className="text-sm font-black text-slate-950">{formatDate(pago.fechaPago)}</div>
                  </div>
                </div>

                <div className="mt-5 rounded-2xl bg-slate-50 p-4">
                  <ReceiptRow label="Socio" value={nombreCompleto} strong />
                  <ReceiptRow label="N° Socio" value={socio.numeroSocio ? `#${socio.numeroSocio}` : "-"} />
                  <ReceiptRow label="Categoría" value={socio.categoria || "-"} />
                  <ReceiptRow label="Concepto" value="Cuota social" />
                ...
                <div className="mt-5 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-center">
                  <div className="text-xs font-bold uppercase tracking-[0.25em] text-blue-700">Monto abonado</div>
                ...
                <div className="mt-5 flex items-center justify-between gap-4 border-t border-slate-200 pt-4 text-xs text-slate-500">
                  <span>Comprobante emitido por {cfg?.nombreClub || "CEDI LOS 15"}</span>
